 ...
 * - retrieves and persists the model via the chartstorage service
...
angular.module('postmvc')
	.controller('ChartCtrl', function ChartCtrl($scope, $window, $routeParams, $filter, store) {
		...
		for (var i = 0; i < 8; i++) {
			$scope.colors[0].push('#333333', '#822f37', '#005b9d', '#7a9ff7', '#37BBE4');
			$scope.colors[1].push('#822f37');
			$scope.colors[2].push('#005b9d');
			$scope.colors[3].push('#7a9ff7');
			$scope.colors[4].push('#37BBE4');
			$scope.colors[5].push('#67a2f7', '#f7941d', '#f9cb08;');
		  	$scope.colors[6].push('#556270', '#4ECDC4', '#C7F464', '#FF6B6B', '#C44D58');
		  	$scope.colors[7].push('#F8B195', '#F67280', '#C06C84', '#6C5B7B', '#355C7D');
		  };
	    ...
	    $scope.lineToggle = function() {
	    	if ($scope.activeLine === true ) {
	    		$scope.activeLine = false;
	    	} else {
	    		$scope.activeLine = true;
	    	}
	    };

	    $scope.pieToggle = function() {
	    	if ($scope.activePie === true ) {
	    		$scope.activePie = false;
	    	} else {
	    		$scope.activePie = true;
	    	}
	    };

	    $scope.columnToggle = function() {
	    	if ($scope.activeColumn === true ) {
	    		$scope.activeColumn = false;
	    	} else {
	    		$scope.activeColumn= true;
	    	}
	    };

	    $scope.areaToggle = function() {
	    	if ($scope.activeArea === true ) {
	    ...
	    $scope.legendToggle = function() {
	    	if ($scope.activeLegend === true ) {
	    		$scope.activeLegend = false;
	    	} else {
	    		$scope.activeLegend = true;
	    	}
	    };

	    $scope.labelToggle = function() {
	    	if ($scope.activeLabel === true ) {
	    		$scope.activeLabel= false;
	    	} else {
	    		$scope.activeLabel = true;
	    	}
	    };
